...
describe("ConditionalTokens", function() {
  ...
  describe("splitting and merging", function() {
    ...
    describe("with EOA trader (Externally Owned Account)", function() {
      let questionId, conditionId;
      const outcomeSlotCount = 2;

      beforeEach(async function() {
        // Setup collateral token
        await collateralToken.mint(eoaTrader.address, collateralTokenCount);
        await collateralToken.connect(eoaTrader).approve(conditionalTokens.address, collateralTokenCount);
      ...
      context("with a condition prepared", function() {
        ...
        context("with valid split", function() {
          ...
          beforeEach(async function() {
            this.splitTx = await conditionalTokens.connect(eoaTrader).splitPosition(
              collateralToken.address,
              NULL_BYTES32,
              conditionId,
              partition,
          ...
          it("should transfer split collateral from trader", async function() {
            expect(await collateralToken.balanceOf(eoaTrader.address)).to.equal(collateralTokenCount.sub(splitAmount));
          ...
          it("should mint amounts in positions associated with partition", async function() {
            for (const indexSet of partition) {
              ...
              expect(await conditionalTokens.balanceOf(eoaTrader.address, positionId)).to.equal(splitAmount);
            }
          });
          ...
          context("with valid merge", function() {
            beforeEach(async function() {
              this.mergeTx = await conditionalTokens.connect(eoaTrader).mergePositions(
                collateralToken.address,
                NULL_BYTES32,
                conditionId,
                partition,
            ...
            it("should transfer split collateral back to trader", async function() {
              expect(await collateralToken.balanceOf(eoaTrader.address)).to.equal(collateralTokenCount.sub(splitAmount).add(mergeAmount));
            ...
            it("should burn amounts in positions associated with partition", async function() {
              for (const indexSet of partition) {
                ...
                expect(await conditionalTokens.balanceOf(eoaTrader.address, positionId)).to.equal(splitAmount.sub(mergeAmount));
              }
            });
          });
          ...
          describe("transferring, reporting, and redeeming", function() {
            ...
            it("should not allow transferring more than split balance", async function() {
              ...
              await expect(
                conditionalTokens.connect(eoaTrader).safeTransferFrom(
                  eoaTrader.address,
            ...
            context("with valid transfer and oracle report", function() {
              beforeEach(async function() {
                ...
                this.transferTx = await conditionalTokens.connect(eoaTrader).safeTransferFrom(
                  eoaTrader.address,
                  counterparty.address,
                  positionId,
                  transferAmount,
              ...
              context("with redemption", function() {
                beforeEach(async function() {
                  this.redeemTx = await conditionalTokens.connect(eoaTrader).redeemPositions(
                    collateralToken.address,
                    NULL_BYTES32,
                    conditionId,
                    partition
                ...
                it("should zero out redeemed positions", async function() {
                  for (const indexSet of partition) {
                    ...
                    expect(await conditionalTokens.balanceOf(eoaTrader.address, positionId)).to.equal(0);
                  }
                });
                ...
                it("should credit payout as collateral", async function() {
                  ...
                  expect(await collateralToken.balanceOf(eoaTrader.address)).to.equal(collateralTokenCount.sub(splitAmount).add(payout));
                });
              });
            });
          });
      ...
      context("with many conditions prepared", function() {
        ...
        context("when trader has collateralized a condition", function() {
          ...
          beforeEach(async function() {
            ...
            await conditionalTokens.connect(eoaTrader).safeTransferFrom(
              eoaTrader.address,
              counterparty.address,
              positionId,
              collateralTokenCount,
          ...
          context("when trader splits to a deeper position with another condition", function() {
            ...
            beforeEach(async function() {
              ...
              this.deepSplitTx = await conditionalTokens.connect(eoaTrader).splitPosition(
                collateralToken.address,
                parentCollectionId,
                conditionId2,
                partition2,
            ...
            it("burns value in the parent position", async function() {
              ...
              expect(await conditionalTokens.balanceOf(eoaTrader.address, positionId)).to.equal(collateralTokenCount.sub(deepSplitAmount));
            });
            ...
            it("mints values in the child positions", async function() {
              for (const indexSet of partition2) {
                ...
                expect(await conditionalTokens.balanceOf(eoaTrader.address, positionId)).to.equal(deepSplitAmount);
              }
            });
          });
